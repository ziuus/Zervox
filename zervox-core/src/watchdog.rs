...
impl Watchdog {
    ...
    async fn handle_heartbeat_connection(
    ...
    async fn run_backup_monitor(&self, peer_addr: String, heartbeat_port: u16) {
        ...
        loop {
            ...
            if self.is_promoted.load(Ordering::Acquire) && self.role == NodeRole::Backup {
                // After promotion we run the listener; this branch is hit if the
                // goroutine somehow loops again, which it shouldn't, but be safe
                continue;
            }
    ...
    async fn promote_to_active(&self, heartbeat_port: u16) {
        ...
        *self.peer_status.write().await = "PRIMARY_DEAD — BACKUP_PROMOTED_TO_LEADER".to_string();
        ...
        tokio::spawn(async move {
            watcher.run_primary_listener(heartbeat_port).await;
        });

        // Note: the main ingestion loop checks watchdog.is_active() before processing
        // each webhook. Because we just set state=Active above, ingest will now
        // route and execute alerts on this instance without any restart required.
    }
}
