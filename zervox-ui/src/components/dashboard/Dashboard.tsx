...
export function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#020409] bg-grid-pattern text-slate-100">
      {/* Ambient top glow */}
      ...
      <main className="mx-auto max-w-screen-2xl px-6 py-8">
        ...
        {isInitializing && (
          <div className="mb-6 flex items-center gap-3 rounded-xl border border-sky-500/20 bg-sky-500/5 px-5 py-3">
        ...
        <section className="mb-8">
          <SectionLabel index="02" title="SYSTEM METRICS" subtitle="Derived from active instance" />
        ...
        <section>
          <div className="mb-4 flex items-center justify-between">
            <SectionLabel index="03" title="INCIDENT FEED" subtitle="Live SQLite WAL · Last 10 incidents" />
            {incidents.length > 0 && (
          ...
          <IncidentTable incidents={incidents} isLoading={!isInitializing && primary.isOnline} />
        </section>

        {/* Footer */}
        <footer className="mt-12 border-t border-[#1e3a5f] pt-6 text-center">
          <p className="font-mono text-[10px] text-slate-700">
            ZERVOX SRE ENGINE v{activeInstance.status?.version ?? '—'} · AUTONOMOUS AIR-GAPPED CYBER RESILIENCE ·{' '}
            <span className="text-sky-900">POLL INTERVAL 3s</span>
          </p>
        </footer>
      </main>
    </div>
